...
import { Calendar, BookOpen, Clock, Users, Edit2, CheckSquare, X, Save } from 'lucide-react';
...
const LecturerTimetableContent = ({ lecturerId }) => {
  ...
  const fetchMyTimetable = async () => {
    try {
      ...
      if (data.success) {
        ...
        const mySlots = data.timeslots.filter(slot => slot.lecturer_id === lecturerId);
  ...
  const fetchBookings = async (timeslotId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/bookings/timeslot/${timeslotId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
      if (data.success) {
  ...
  const handleMarkAttendance = async (bookingId, status) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/bookings/${bookingId}/attendance`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm">
          ...
          <span className="text-sm font-medium">{timeslots.length} Sessions</span>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div className="text-sm">
              ...
              <span className="ml-2 font-medium">{selectedSlot.seat_count}</span>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notice (Students will be notified)</label>
                <textarea
                  value={editNotice}
