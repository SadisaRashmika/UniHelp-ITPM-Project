...
const AdminTimetableContent = () => {
  ...
  const handleAddTimeslot = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/admin/timeslots`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateTimeslot = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/admin/timeslots/${editingSlot.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleDeleteTimeslot = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/admin/timeslots/${id}`, {
        method: 'DELETE',
