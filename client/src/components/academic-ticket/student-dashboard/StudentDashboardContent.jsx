...
const StudentDashboardContent = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const quizzesResponse = await fetch('/api/academic-ticket/quizzes/student');
        if (quizzesResponse.ok) {
          const quizzesData = await quizzesResponse.json();
          const quizDeadlines = quizzesData.quizzes.map((quiz, index) => ({
            id: quiz.id,
            title: quiz.title,
            course: quiz.course_code,
            type: "quiz",
            dueDate: new Date(quiz.due_date).toLocaleDateString('en-US', { 
              year: 'numeric', 
              month: 'short', 
              day: 'numeric' 
            }),
            dueTime: new Date(quiz.due_date).toLocaleTimeString('en-US', { 
              hour: '2-digit', 
              minute: '2-digit' 
            }),
          ...
          // Mock assignments and practicals for now
          const mockDeadlines = [
            {
              id: 999,
          ...
          const allDeadlines = [...quizDeadlines, ...mockDeadlines];
  ...
  const handleStartQuiz = (quizId) => {
    ...
    window.location.href = `/student-dashboard/quiz/${quizId}`;
  };
  ...
  const handleRegisterCourse = () => {
    console.log('Opening course registration');
    // Navigate to course registration page
    window.location.href = '/student-dashboard/register';
  };
              ...
              >
                ...
                {deadline.type === "quiz" && (
                  <div className="text-center">
                    <button 
                      onClick={() => handleStartQuiz(deadline.id)}
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className="p-4 bg-blue-100 rounded hover:bg-blue-200 transition-colors"
              >
                Register Course
              </button>
              <button 
                onClick={handleSubmitAssignment}
                className="p-4 bg-green-100 rounded hover:bg-green-200 transition-colors"
              >
                Submit Assignment
              </button>
            </div>
