...
const PracticalForm = ({ onClose, onPracticalCreated }) => {
  ...
  const validate = () => {
    ...
    if (!formData.duration || Number(formData.duration) <= 0) errs.duration = "Duration must be greater than 0";
    if (!formData.max_marks || Number(formData.max_marks) <= 0) errs.max_marks = "Marks must be greater than 0";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        total_marks: Number(formData.max_marks),
        module_id: Number(formData.module_id),
        type: 'practical',
        lecturer_id: formData.lecturer_id
      };

      const response = await axios.post(`${API_BASE}/quizzes`, payload);
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              onChange={handleInputChange}
              rows={4}
              className={`w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl transition-all outline-none font-medium resize-none ${validationErrors.description ? 'border-red-500' : ''}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={handleInputChange}
                className={`w-full px-5 py-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-xl transition-all outline-none font-bold text-sm ${validationErrors.due_date ? 'border-red-500' : ''}`}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={handleInputChange}
                className={`w-full px-5 py-3 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-xl transition-all outline-none font-bold text-sm ${validationErrors.max_marks ? 'border-red-500' : ''}`}
              />
            </div>
